...
class TestTokenizerWithDictionary(unittest.TestCase):
    ...
    def testSerialize2(self):
        token = AT(text='있는', dictionary_form='있다', definition='', pos=POS_AUXILIARY, decomposed_tokens=[D(POS_ENDING, '는')])
        res = token.jsonify()
    ...
    def testMekabTokenizer(self):
        ...
        [AT(text='살고', dictionary_form='살다', definition='', pos=POS_VERB, decomposed_tokens=[D(POS_ENDING, '고')]),
         WS(),
         AT(text='있는', dictionary_form='있다', definition='', pos=POS_AUXILIARY, decomposed_tokens=[D(POS_ENDING, '는')]),
         WS(),
    ...
    def testTokenizeFunc(self):
        ...
        expected_glossary = {'전혀', '무관', '하', '보이다', '그런데', '아들', '이'}
       # self.assertEqual(glossary, expected_glossary) # because of 다 ending
